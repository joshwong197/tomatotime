...
import { CompletedSession, SessionType, ArchivedSeed } from '../types';
...
export const HistoryHub: React.FC<HistoryHubProps> = ({ history, archivedSeeds, onClose }) => {
  const getSeedFocusTime = (seedId?: string): number | null => {
    if (!seedId) return null;
    const seed = archivedSeeds.find(s => s.id === seedId);
    return seed?.focusTime ?? null;
  };

  const exportToCSV = () => {
    const headers = ['Date', 'Task', 'Type', 'Start Time', 'End Time', 'Duration (Min)', 'Task Focus Time'];
    const rows = history.map(s => {
      const ft = getSeedFocusTime(s.seedId);
      return [
        s.date,
        s.label.replace(/,/g, ' '),
        s.type,
        ...
        s.durationMinutes,
        ft != null && ft > 0 ? formatFocusTime(ft) : ''
      ];
    });
              ...
              {(Array.from(new Set(history.map(s => s.date))) as string[]).sort((a, b) => new Date(b).getTime() - new Date(a).getTime()).map(date => (
                <div key={date} className="space-y-3">
                  ...
                  {history.filter(s => s.date === date).map(session => {
                    const ft = session.type === SessionType.FOCUS ? getSeedFocusTime(session.seedId) : null;
                    const ftDisplay = ft != null && ft > 0 ? formatFocusTime(ft) : null;
                    return (
                      <div key={session.id} className="bg-white p-5 rounded-3xl border-2 border-stone-100 shadow-sm flex items-center justify-between hover:border-red-200 transition-colors">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-[10px] text-stone-400 font-black uppercase tracking-widest">
                              ...
                              {ftDisplay && (
                                <>
                                  <span>•</span>
                                  <span className="text-red-400">⏱ {ftDisplay} on task</span>
                                </>
                              )}
                            </div>
                          </div>
